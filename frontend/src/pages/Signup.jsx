...
export default function Signup() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      toast.success("Welcome to Velari.");
    ...
    <div className="min-h-screen grid lg:grid-cols-2 bg-velari-bg">
      <div className="hidden lg:flex relative overflow-hidden border-r border-velari-border bg-velari-ink text-velari-cream">
        ...
        <div className="relative m-auto px-16 max-w-lg">
          <Logo size={32} className="text-velari-cream" />
          <h2 className="font-display text-4xl mt-12 leading-tight">
            A daily ritual that gives you your life back.
          </h2>
          <ul className="mt-9 space-y-3.5 text-[15px]">
            {BENEFITS.map((b) => (
              <li key={b} className="flex items-start gap-3 opacity-90">
                <Check size={16} className="mt-0.5 text-velari-brand" />
                {b}
              </li>
            ))}
          </ul>
          <p className="font-editorial italic text-[15px] mt-12 opacity-70">
            "It is the first productivity app that makes me feel quieter, not busier."
            <br />— Marina K., Designer
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col">
        <div className="px-6 lg:px-10 py-6 flex items-center justify-between">
          ...
          <div className="ml-auto text-[13px] text-velari-textSoft">
            Already with us?{" "}
            <Link to="/login" className="text-velari-text underline underline-offset-4">Sign in</Link>
          </div>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center px-6">
          <div className="w-full max-w-sm">
            ...
            <form onSubmit={submit} className="space-y-3" data-testid="signup-form">
              ...
              <Field label="Email" value={email} onChange={setEmail} type="email" testid="signup-email" />
              <Field label="Password" value={password} onChange={setPassword} type="password" testid="signup-password" />
              {error && (
                <div className="flex items-start gap-2 text-[13px] text-red-700 bg-red-50 border border-red-100 rounded-xl px-3 py-2.5" data-testid="signup-error">
